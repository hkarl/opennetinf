...
public class RemoteNodeConnectionTest {
	...
	this.injector = Guice.createInjector(new CommonTestModule(
		this.properties));

	this.messageEncoder = this.injector
		.getInstance(MessageEncoderProtobuf.class);
	this.datamodelFactory = this.injector
		.getInstance(DatamodelFactory.class);

	final Properties node2properties = Utils
		.loadProperties(NETINFNODE_PROPERTIES);
	// Guice.createInjector(new LogModule(node2properties));
	this.createInjector = Guice.createInjector(new SecurityModule(),
		new DatamodelImplModule(), new AbstractModule() {

		    @Override
		    protected void configure() {
			Names.bindProperties(binder(), node2properties);
			bind(NetInfNodeConnection.class).annotatedWith(
				SecurityModule.Security.class).to(
				RemoteNodeConnection.class);
		    }
		});
	datamodelFactoryN2 = createInjector.getInstance(DatamodelFactory.class);

	IdentifierLabel testIdentifierLabel = this.datamodelFactory
	...
	IdentifierLabel testIdentifierLabel2 = this.datamodelFactory
		.createIdentifierLabel();
	...
	IdentifierLabel testIdentityLabel = this.datamodelFactory
		.createIdentifierLabel();
	...
	this.testIO = DatamodelTest
		.createDummyInformationObject(this.datamodelFactory);
	...
	RemoteNodeConnection rnc = this.createInjector
		.getInstance(RemoteNodeConnection.class);
	try {
